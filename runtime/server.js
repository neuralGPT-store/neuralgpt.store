...
const route = createRouter(listingsHandlers, stripeHandlers, alertsHandlers); const server = http.createServer(async (req, res) => { try { applyCors(req, res, env.corsOrigin); if (String(req.method || '').toUpperCase() === 'OPTIONS') { res.statusCode = 204; return res.end(); } const url = parseUrl(req); return await route(req, res, url); } catch (error) { return sendError(res, 500, 'runtime_unhandled_error', error.message || 'unknown'); }
