...
function parseUrl(req) {
  return new URL(req.url, 'http://runtime.local');
}
...
async function readMultipartBody(req, maxBytes) {
  const contentType = String(req.headers['content-type'] || '');
  const match = contentType.match(/boundary=([^;]+)/i);
  if (!match || !match[1]) throw new Error('multipart_boundary_missing');
  ...
  while (cursor < raw.length) {
    let partStart = raw.indexOf(delimiter, cursor);
    if (partStart < 0) break;
    if (raw.indexOf(closeDelimiter, partStart) === partStart) break;
    partStart += delimiter.length;
    if (raw[partStart] === 13 && raw[partStart + 1] === 10) partStart += 2;

    const nextBoundary = raw.indexOf(delimiter, partStart);
